...
describe('EditorPanel', () => {
  ...
  describe('run canvas button', () => {
    it('should render run canvas button when onRunCanvas is provided', () => {
      // Arrange & Act
      render(<EditorPanel {...defaultProps} onRunCanvas={vi.fn()} />)

      // Assert
      expect(screen.getByRole('button', { name: /run canvas/i })).toBeInTheDocument()
    })

    it('should not render run canvas button when onRunCanvas is not provided', () => {
      // Arrange & Act
      render(<EditorPanel {...defaultProps} />)

      // Assert
      expect(screen.queryByRole('button', { name: /run canvas/i })).not.toBeInTheDocument()
    })

    it('should call onRunCanvas when run canvas button is clicked', () => {
      // Arrange
      const onRunCanvas = vi.fn()
      render(<EditorPanel {...defaultProps} onRunCanvas={onRunCanvas} />)

      // Act
      fireEvent.click(screen.getByRole('button', { name: /run canvas/i }))

      // Assert
      expect(onRunCanvas).toHaveBeenCalledTimes(1)
    })

    it('should disable run canvas button when code is empty', () => {
      // Arrange & Act
      render(<EditorPanel {...defaultProps} code="" onRunCanvas={vi.fn()} />)

      // Assert
      expect(screen.getByRole('button', { name: /run canvas/i })).toBeDisabled()
    })

    it('should disable run canvas button when code is whitespace only', () => {
      // Arrange & Act
      render(<EditorPanel {...defaultProps} code="   " onRunCanvas={vi.fn()} />)
    ...
    it('should enable run canvas button when code has content', () => {
      // Arrange & Act
      render(<EditorPanel {...defaultProps} code='print("hello")' onRunCanvas={vi.fn()} />)

      // Assert
      expect(screen.getByRole('button', { name: /run canvas/i })).not.toBeDisabled()
    })

    it('should render both format and run canvas buttons when both provided', () => {
      // Arrange & Act
      render(<EditorPanel {...defaultProps} onFormat={vi.fn()} onRunCanvas={vi.fn()} />)
