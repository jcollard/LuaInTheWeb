...
- `data` (table): Raw RGBA pixel data as a flat array (1-indexed in Lua)

The data array contains 4 values per pixel (red, green, blue, alpha) in row-major order:
...
**Performance Note:** Pixel manipulation can be slow for large images. For real-time effects, keep image sizes small (e.g., 100x100 or less) or process only portions of the canvas.
